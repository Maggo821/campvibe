...
import { updatePlaceAction, type UpdatePlaceState } from "@/app/places/[id]/edit/actions";
...
const initialState: UpdatePlaceState = {
  success: false,
  message: "",
};
...
export function PlaceEditForm({ initialData, features }: PlaceEditFormProps) {
  const [state, action, pending] = useActionState(updatePlaceAction, initialState);
  ...
  return (
    <form action={action} className="space-y-6">
      ...
      {state.message ? (
        <p className={`rounded-xl px-3 py-2 text-sm ${state.success ? "bg-emerald-100 text-emerald-900" : "bg-rose-100 text-rose-900"}`}>
          {state.message}
        </p>
      ) : null}
