...
import { NewNearbyPlaceForm } from "@/components/places/NewNearbyPlaceForm";
import { hasSupabaseEnv } from "@/lib/supabase/env";
import { getSupabaseServerClient } from "@/lib/supabase/server";

export default function NewNearbyPlacePage({ params }: { params: Promise<{ id: string }> }) {
  return <NewNearbyPlaceContent params={params} />;
}

async function NewNearbyPlaceContent({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;

  if (!hasSupabaseEnv()) {
    return (
      <main className="rounded-[2rem] border border-black/10 bg-white/80 p-6 shadow-sm">
        <h1 className="text-2xl font-semibold">Ort in der Nähe hinzufügen</h1>
    ...
    <main className="rounded-[2rem] border border-black/10 bg-white/80 p-6 shadow-sm">
      ...
      {!editable ? (
        <p className="mt-4 rounded-2xl bg-amber-100 px-4 py-3 text-sm text-amber-900">
          Du kannst aktuell nur Nearby-Orte auf eigenen Plätzen anlegen.
        </p>
      ) : (
        <div className="mt-6">
          <NewNearbyPlaceForm placeId={id} />
        </div>
      )}
    </main>
  );
